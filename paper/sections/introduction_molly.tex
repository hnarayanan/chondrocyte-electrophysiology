...
<<<<<<< local
\todoM{We need to put all references in bib.}
Articular cartilage 
...
=======
\todo{{\bf Harish:} Will add citations and address other issues noted
  by Molly.}
...
\todoM{I merged in the final paragraph and commented this (and some other stuff) out, so this first paragraph should be checked and edited.}
...
%chondrocyte}---which is responsible for synthesis and homeostasis of
%the matrix.  
%Chondrocytes, like all other cells,
%must possess effective membrane transport systems to minimize changes
...
Hall, 2003) and limited recovery \M{Recovery from what? Specify.} of chondrocytes (Jones et al, 1999)
...
membrane potential in these cells \citep{Lewisetal2011}.  In unhealthy \M{Can we be more specific? If not, we perhaps need to stay with "abnormal".}
...
studies (cite?). \M{Not sure how to cite this.} We have thus developed and present here a detailed,
...
=======
\todo{{\bf Molly:} Merge the following notes further motivating the
  model with the preceding introductory text.}
