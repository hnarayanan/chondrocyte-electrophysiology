...
\todo{{\bf Harish:} Rearranged text. Will address issues and polish.}
...
\todo{{\bf Harish:} Make sure the following figure is updated to match
  the final selection of channels}

\subsection*{Formulations ionic currents}
\label{sec:formulation-ionic-current}

\todo{{\bf Harish:} Clean up the following mishmash of notes and equations}
...
\todo{This channel is a primary calcium concentration regulation
  mechanism. Need to explain the importance of this.}
...
\todo{Need to explain why we choose the Na-H exchanger over a
  possible ASIC channel}
...
\todo{We currently model Kv1.5. Need to replace this with Kv1.4 from,
  e.g., Nygren et al.}
...
\todo{Do we need to model this. Can this be linked to $I_{Na_{b}}$?}
...
\todo{Discuss details of the solution process here. Talk about the
  methods used \citep{RadhakrishnanHindmarsh1993}, parametrisation of
  the model and the fact that the corresponding code is available for
  free for anyone to test and extend.}

\subsection*{The atypical environment of the chondrocyte (how
  parameters are chosen)}
\label{sec:chondrocyte-environment}

\todo{Polish the notes below to motivate the choice of external
